...
def _estimate_tokens(text: str) -> int:
    """Estimate token count using tiktoken when available, else a word-based heuristic.

    The heuristic counts whitespace-delimited words and applies a 1.3x
    multiplier (English text averages ~0.75 words per token).  This is
    significantly more accurate than the previous ``len(text) // 4`` approach.
    """
    try:
        import tiktoken
...
def format_memories_for_context(memories: List[Dict], max_tokens: int = 4000) -> str:
    ...
    if _estimate_tokens(context) > max_tokens:
        # Binary-search for a safe truncation point
        lo, hi = 0, len(context)
        while lo < hi:
            mid = (lo + hi + 1) // 2
            if _estimate_tokens(context[:mid]) <= max_tokens - 20:
                lo = mid
            else:
                hi = mid - 1
...
async def chat_with_memories(
    ...
    # Estimate total token usage (prompt + response)
    prompt_text = " ".join(m["content"] for m in messages)
    tokens_used = _estimate_tokens(prompt_text) + _estimate_tokens(response_text)
