...
async def list_audit_entries(
    ...
    cursor: Optional[str] = Query(None, description="Cursor (entry_id) — fetch entries after this ID"),
    ...
    When ``cursor`` is supplied the response starts after that entry_id.
    """
    _ensure_loaded()

    # Determine the starting index when a cursor is supplied
    cursor_start: Optional[int] = None
    if cursor:
        for idx, e in enumerate(reversed(_audit_entries)):
            if e.entry_id == cursor:
                cursor_start = idx + 1
                break
    ...
    effective_offset = 0 if cursor_start is not None else offset

    for idx, entry in enumerate(reversed(_audit_entries)):
        # Skip entries before cursor
        if cursor_start is not None and idx < cursor_start:
            continue
    ...
    next_cursor = page[-1].entry_id if len(page) == limit and page else None
