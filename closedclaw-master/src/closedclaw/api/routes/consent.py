...
async def respond_to_consent(
    ...
    # Persist to SQLite first (source of truth), then update in-memory cache
    store = get_persistent_store()
    try:
        store.save_consent_receipt(receipt.model_dump())
    except Exception as e:
        logger.error(f"Failed to persist consent receipt: {e}")
        raise HTTPException(status_code=500, detail="Failed to persist consent receipt")
    _consent_receipts[receipt.receipt_id] = receipt

    # Remove from pending (SQLite first, then in-memory)
    try:
        store.delete_pending_consent(request_id)
    except Exception as e:
        logger.error(f"Failed to delete pending consent from storage: {e}")
...
async def list_consent_receipts(
    ...
    cursor: Optional[str] = Query(None, description="Cursor (receipt_id) — fetch receipts after this ID"),
    token: str = Depends(get_auth_token),
):
    """List consent receipts with optional filtering and cursor pagination."""
    _ensure_loaded()
    ...
    # Apply cursor: skip everything up to and including the cursor receipt
    if cursor:
        found = False
        for idx, r in enumerate(receipts):
            if r.receipt_id == cursor:
                receipts = receipts[idx + 1 :]
                found = True
                break
        if not found:
            receipts = []
    ...
    page = receipts[:limit]
    next_cursor = page[-1].receipt_id if len(page) == limit and page else None
...
def create_consent_request(
    ...
    try:
        ...
        logger.error(f"Failed to persist pending consent request: {e}")
        raise
