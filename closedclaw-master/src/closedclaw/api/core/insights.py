...
# =============================================================================
# FACTORY (worker-safe — each worker/thread gets its own instance)
# =============================================================================
...
def get_insight_engine(memory=None, settings=None) -> InsightEngine:
    """Get or create a worker-local InsightEngine instance.

    Using thread-local storage ensures each Uvicorn/Gunicorn worker
    (and each thread within a worker) gets its own ``InsightEngine``,
    avoiding shared mutable state across concurrent requests.
    """
    engine: Optional[InsightEngine] = getattr(_thread_local, "engine", None)
    if engine is None:
        engine = InsightEngine(memory=memory, settings=settings)
