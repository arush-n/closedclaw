...
Enforces user-defined privacy rules via the canonical PolicyEngine with:

1. Retrieve: Get relevant memories from vault
2. Classify: Confirm/upgrade sensitivity levels
3. Rule Match: Evaluate against policy rules (PolicyEngine)
...
def _action_from_policy(action: PolicyAction) -> FirewallAction:
    """Map a PolicyAction (from PolicyEngine) to the internal FirewallAction."""
    _MAP = {
        PolicyAction.PERMIT: FirewallAction.PERMIT,
        PolicyAction.REDACT: FirewallAction.REDACT,
        PolicyAction.BLOCK: FirewallAction.BLOCK,
        PolicyAction.CONSENT_REQUIRED: FirewallAction.CONSENT_REQUIRED,
    }
    return _MAP.get(action, FirewallAction.BLOCK)
...
class PrivacyFirewall:
    ...
    def __init__(
        ...
        Args:
            policy_engine: PolicyEngine instance for rule evaluation
    ...
    def policy_engine(self) -> PolicyEngine:
        """Get the policy engine (creates default if not provided)."""
        if self._policy_engine is None:
            self._policy_engine = PolicyEngine(PolicySet(**DEFAULT_POLICIES))
    ...
    async def process(
        ...
        for ctx in contexts:
            ...
            # Stage 2: Rule matching via PolicyEngine
            mem_dict = {
                "id": ctx.memory_id,
                "memory": ctx.content,
            ...
            policy_action, matched_rule = self.policy_engine.evaluate(
                mem_dict, provider,
            ...
            if matched_rule:
                ctx.rule_matched = matched_rule.id
                ctx.action = _action_from_policy(policy_action)
                if matched_rule.id not in seen_rules:
                    seen_rules.add(matched_rule.id)
                    rules_evaluated.append(matched_rule.id)
            else:
    ...
    def evaluate_memory(
        ...
        # Rule matching via PolicyEngine
        mem_dict = {
            "sensitivity": ctx.effective_sensitivity,
            "tags": ctx.tags,
            "source": ctx.source,
    ...
    def test_rule(
        ...
        for mem in test_memories:
            ...
            # Use PolicyEngine's matching logic for the specific rule
            test_mem = {
                "sensitivity": ctx.effective_sensitivity,
                "tags": ctx.tags,
                "source": mem.get("source", "conversation"),
            }
            matches = self.policy_engine._matches(rule, test_mem, provider)
